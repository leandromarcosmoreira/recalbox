...
do_update() {
  ...
  # Mount
  echo "mount image"
  displayImage "$IMAGE_PATH/offline-install-2.jpg" "inf"
  sleep 1
  ...
  displayImage "$IMAGE_PATH/offline-install-3.jpg" "inf"
  sleep 1
  mount -o remount,rw /boot/ || return 4

  # Files copy
  echo "copying update boot files"
  displayImage "$IMAGE_PATH/offline-install-4.jpg" "inf"
  ...
  displayImage "$IMAGE_PATH/offline-install-5.jpg" "inf"
  sleep 1

  # Umount
  echo "unmount image"
  umount /mnt/ || return 8
  displayImage "$IMAGE_PATH/offline-install-6.jpg" "inf"
  sleep 1
...
if [ $RC -eq 0 ]; then
  ...
  displayImage "$IMAGE_PATH/offline-install-7.jpg" "inf"
  sleep 1
else
  echo "upgrade failed return code $RC" >&2
  displayImage "$IMAGE_PATH/offline-install-error.jpg" "inf"
  sleep 10
fi
sync
reboot
