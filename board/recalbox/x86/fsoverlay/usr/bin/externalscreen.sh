...
# Configure output for pinball
CABINET=$(recalbox_settings -command load -key pinball.cabinet)
if [[ "$CABINET" -eq 1 ]]; then
  PREFERED=$(recalbox_settings -command load -key pinball.screen1)
  FORCE_RES=$(recalbox_settings -command load -key pinball.screen1.resolution)
fi
