...
def setVideoMode(videoMode: str, delay=0.5):
    # The user mentioned default for the videomode
...
def getCrtResolution(system: Emulator) -> str|None:
    res = getCrtResolutionDetail(system)
    if res is None:
        return None
    return "{}x{}{}".format(res[0], res[1], "i" if res[2] else "")

### Return the width, height and if mode is interlaced (True = interlaced, False = progressive)
def getCrtResolutionDetail(system) -> (int, int, bool):
    from configgen.crtswitchres.CRTTypes import CRTResolutionType, CRTScreenType
    if system.CRTAdapter == "none":
        return None
    match system.CRTScreenType:
        case system.CRTScreenType.kHz15:
            if system.CRTResolutionType == CRTResolutionType.Force480 and system.CRTAdapter.supportsInterlaced():
                return 640, 480, True
            return 320, 240, False
        ...
        case CRTScreenType.kHzMulti1525:
            ...
            if system.CRTResolutionType == CRTResolutionType.Force480 and system.CRTAdapter.supportsInterlaced():
                return 640, 480, True
            return 496, 384, False
        case CRTScreenType.kHzMulti1531:
            if system.CRTResolutionType == CRTResolutionType.Force480 or system.CRTResolutionType == CRTResolutionType.Auto:
                return 640, 480, False
            elif system.CRTResolutionType == CRTResolutionType.Force240:
                return 320, 240, False
            elif system.CRTResolutionType == CRTResolutionType.Force480i and system.CRTAdapter.supportsInterlaced():
                return 640, 480, True
            else:
                return 640, 480, False
        case CRTScreenType.kHzTriFreq:
            if system.CRTResolutionType == CRTResolutionType.Force240:
                return 320, 240, False
            if system.CRTResolutionType == CRTResolutionType.Force384:
                return 496, 384, False
                ...
                if system.Name == "model3" and  system.CRTResolutionType == CRTResolutionType.Auto:
                    return 496, 384, False
                return 640, 480, False
        case _:
            return None

def createCrtDesktopRes(res, system):
    from configgen.crtswitchres.CRTTypes import CRTAdapter
    if system.CRTEnabled and res:
        with open("/tmp/crtdesktopres", "a") as f:
            f.write(res)

def deleteCrtDesktopRes():
    if os.path.exists("/tmp/crtdesktopres"):
        os.remove("/tmp/crtdesktopres")
