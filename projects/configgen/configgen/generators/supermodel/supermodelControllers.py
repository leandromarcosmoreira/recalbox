...
def generateControllerConfig(self, playersControllers: ControllerPerPlayer):
    ...
    controlsFile = recalboxFiles.CONF + '/model3/Supermodel.ini'
    ...
    SERVICE_TEST_BUTTON: Dict[str, int] = {
        "InputServiceA":            InputItem.ItemL3,
        "InputTestA":               InputItem.ItemR3,
    }
    ...
    MOUSE_GAME: Dict[str, int] = {
        ...
        # Trigger for gun
        "InputTrigger":             InputItem.ItemA,
        "InputAnalogJoyTrigger":    InputItem.ItemA,
        "InputAnalogJoyEvent":      InputItem.ItemB,
        "InputAnalogTriggerLeft":   InputItem.ItemL2,
        "InputAnalogTriggerRight":  InputItem.ItemR2,
    }

    GUNCON_JOY: Dict[str, str] = {
        # Light guns (Lost World)
        "InputGunX":                "XAXIS",
        "InputGunY":                "YAXIS",
        # Analog guns (Ocean Hunter, LA Machineguns)
        "InputAnalogGunX":          "XAXIS",
        "InputAnalogGunY":          "YAXIS",
        # Analog joystick (Star Wars Trilogy)
        "InputAnalogJoyX":          "XAXIS",
        "InputAnalogJoyY":          "YAXIS",
        # Trigger for gun
        "InputTrigger":             "BUTTON6",
        "InputAnalogJoyTrigger":    "BUTTON6",
        "InputAnalogJoyEvent":      "BUTTON8",
        "InputAnalogTriggerLeft":   "BUTTON6",
        "InputAnalogTriggerRight":  "BUTTON8",
    }
    ...
    JAMMA_CONTROL_P1: Dict[str, int] = {
        # Fighting game buttons
        "InputGuard":               InputItem.ItemY,
        "InputPunch":               InputItem.ItemA,
        "InputKick":                InputItem.ItemB,
        "InputEscape":              InputItem.ItemX,
        # Spikeout buttons
        "InputShift":               InputItem.ItemY,
        "InputBeat":                InputItem.ItemA,
        "InputCharge":              InputItem.ItemB,
        "InputJump":                InputItem.ItemX,
        # Virtua Striker buttons
        "InputShortPass":           InputItem.ItemA,
        "InputLongPass":            InputItem.ItemB,
        "InputShoot":               InputItem.ItemX,
        # Pedals
        "InputAccelerator":         InputItem.ItemA,
        "InputBrake":               InputItem.ItemB,
        # Handbrake (Dirt Devils, Sega Rally 2)
        "InputHandBrake":           InputItem.ItemX,
        # Single view change button (Dirt Devils, ECA, Harley-Davidson, Sega Rally 2)
        "InputViewChange":          InputItem.ItemY,
        # VR4 view change buttons (Daytona 2, Le Mans 24, Scud Race)
        "InputVR1":                 -1,
        "InputVR2":                 -1,
        "InputVR3":                 -1,
    ...
    def getConfigValue(inputItem: InputItem, targetConf: Dict[str, int]):
        ...
        # Output format RYAXIS, RXAXIS, XAXIS, YAXIS
        if inputItem.IsAxis:
            if x in targetConf:
    ...
    def getlightGunConfigValue(inputItem: InputItem):
        ...
        if inputItem.IsAxis:
            return 'JOY{}_{}'.format(pad.SdlIndex + 1, SUPERMODEL_JOY[MOUSE_GAME[x]])
        else:
            return ''
    ...
    supermodelControllersSettings = IniSettings(controlsFile, True)
    supermodelControllersSettings.loadFile(True)
    from configgen.settings.keyValueSettings import keyValueSettings
    supermodelSettings = keyValueSettings(configFile)
    ...
    for playercontroller in playersControllers:
        ...
        is_jamma = pad.DeviceName.startswith("JammaController")
        is_guncon = pad.DeviceName.startswith("Namco GunCon 2")
        ...
        if is_jamma:
            for x in JAMMA_CONTROL_P1:
                if JAMMA_CONTROL_P1[x] != -1:
                    print("CONFIGURE P1")
                    supermodelControllersSettings.setString(self.SECTION_GLOBAL, x, '"{}_BUTTON{}"'.format(padIndex, JAMMA_CONTROL_P1[x] + 1))
                else:
                    supermodelControllersSettings.setString(self.SECTION_GLOBAL, x, '')
        ...
        if is_guncon:
            for x in GUNCON_JOY:
                supermodelControllersSettings.setString(self.SECTION_GLOBAL, x, '"{}_{}"'.format(padIndex, GUNCON_JOY[x]))
        ...
        if pad.PlayerIndex == 2 and is_guncon:
            padIndexj1 = 'JOY{}'.format(pad.SdlIndex + 1)
            for x in GUNCON_JOY:
                supermodelControllersSettings.setString(self.SECTION_GLOBAL, x, '"{}_{},{}_{}"'.format(padIndexj1, GUNCON_JOY[x], padIndex, GUNCON_JOY[x]))

        if pad.PlayerIndex == 2 and is_jamma:
            for x in JAMMA_CONTROL_P2:
                if JAMMA_CONTROL_P2[x] != -1:
                    print("CONFIGURE P2")
                    supermodelControllersSettings.setString(self.SECTION_GLOBAL, "{}2".format(x), '"{}_BUTTON{}"'.format(padIndex, JAMMA_CONTROL_P2[x] + 1))
                else:
                    supermodelControllersSettings.setString(self.SECTION_GLOBAL, x, '')
