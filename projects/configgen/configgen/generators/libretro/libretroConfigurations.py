...
class LibretroConfiguration:
    ...
    def __init__(self, system: Emulator, controllers: ControllerPerPlayer, rom: str, demo: bool, nodefaultkeymap: bool, recalboxOptions: keyValueSettings):
        ...
        self.remapSettings = keyValueSettings(recalboxFiles.retroarchRemapCommon, True)
    ...
    def loadRetroarchConfigurations(self) -> (keyValueSettings, keyValueSettings):
        ...
        self.remapSettings.loadFile()
        return retroarchConfig, self.remapSettings
    ...
    def createRetroarchConfiguration(self) -> (keyValueSettings, keyValueSettings, keyValueSettings):
        # Load settings
        retroarchConfig, retroarchRemap = self.loadRetroarchConfigurations()
        ...
        controllers = LibretroControllers(self.system, self.recalboxOptions, retroarchConfig, self.coreSettings, retroarchRemap, self.controllers, self.nodefaultkeymap, self.rom)
        controllers.fillControllersConfiguration()

        # Save settings
        retroarchConfig.changeSettingsFile(self.retroarchCustomFile)
        retroarchConfig.saveFile()
        self.coreSettings.saveFile()
        ...
        # Save remap
        retroarchRemap.saveFile()

        # Return file for testing purpose only
        return retroarchConfig, retroarchOverrides, retroarchRemap
