...
from configgen.crtswitchres.CRTTypes import CRTVideoStandard, CRTRegion, CRTScreenType, CRTResolutionType
...
VideoStandardToRegionMap = {
    ...
    "gearsystem":
        {
            "prop_name": "gearsystem_timing",
            "values": {CRTVideoStandard.AUTO: '"Auto"', CRTVideoStandard.NTSC: '"NTSC (60 Hz)"', CRTVideoStandard.PAL: '"PAL (50 Hz)"'}
        }
}
...
class LibretroCoreConfigCRTSwitchres:
    ...
    def createConfigFor(self, system: Emulator) -> typing.Dict[str, str]:
        ...
        lines["dolphin_efb_height"] = '480'
        # if system.Name == "mastersystem"
        lines["gearsystem_overscan"] = '"Full (284 width)"'

        if system.Core == "cap32":
            if system.CRTResolutionType != CRTResolutionType.Force240 and system.CRTScreenType >= CRTScreenType.kHz31:
                lines["cap32_scr_crop"] = '"enabled"'
            else:
                lines["cap32_scr_crop"] = '"disabled"'
        if system.Core == "dosbox_pure":
            if system.CRTVideoStandard == CRTVideoStandard.AUTO or system.CRTVideoStandard == CRTVideoStandard.NTSC:
                lines["dosbox_pure_force60fps"] = '"true"'
            else:
                lines["dosbox_pure_force60fps"] = '"50"'
        if system.Core == "scummvm":
            if system.CRTVideoStandard == CRTVideoStandard.AUTO or system.CRTVideoStandard == CRTVideoStandard.NTSC:
                lines["scummvm_framerate"] = '"60 Hz"'
            else:
                lines["scummvm_framerate"] = '"50 Hz"'
        #
