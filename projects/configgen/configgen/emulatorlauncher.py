...
def getGenerator(emulator):
    ...
    if emulator == "advancemame":
    ...
    elif emulator == "corsixth":
        module = __import__("configgen.generators.corsixth.corsixthGenerator", fromlist=["CorsixTHGenerator"])
        generatorClass = getattr(module, "CorsixTHGenerator")
    ...
    elif emulator == "julius":
        module = __import__("configgen.generators.julius.juliusGenerator", fromlist=["JuliusGenerator"])
        generatorClass = getattr(module, "JuliusGenerator")
    elif emulator == "kodi":
    ...
    elif emulator == "openlara":
        module = __import__("configgen.generators.openlara.openlaraGenerator", fromlist=["OpenlaraGenerator"])
        generatorClass = getattr(module, "OpenlaraGenerator")
        ...
        module = __import__("configgen.generators.pico8.pico8Generator", fromlist=["Pico8Generator"])
        generatorClass = getattr(module, "Pico8Generator")
    else:
...
def getDefaultEmulator(systemName: str, emulatorName: str, coreName: str, videoMode: str, ratio: str):
    ...
    emulator = Emulator(name=systemName, emulator=emulatorName, core=coreName, videoMode=videoMode, ratio=ratio)
    return emulator
...
def main(arguments) -> (int, bool):
    ...
    system = getDefaultEmulator(systemName, arguments.emulator, arguments.core, arguments.resolution, arguments.ratio)
    recalboxOptions, fixedScreenSize = loadRecalboxSettings(arguments.rom, system.Name)
    recalboxCrtOptions = keyValueSettings("/boot/crt/recalbox-crt-options.cfg", True)
    recalboxCrtOptions.loadFile()
    ...
    if system is not None:
        ...
        system.configure(recalboxOptions, arguments, recalboxCrtOptions)
...
if __name__ == '__main__':
    ...
    parser.AddString(command="crtsignaltype", chelp="RGB or Composite", required=False)
    ...
    parser.AddInt(command="lightgunluminosity", chelp="lightgun luminosity boost", required=False)
    ...
    parser.AddSimple(command="hdgame", chelp="jamma layout for controls", required=False)
