...
void ResolutionAdapter::GetMaximumResolution(int& w, int& h, bool& strict)
{
  switch(Board::Instance().GetBoardType())
  {
    ...
    case BoardType::Pi400:
    case BoardType::Pi5:
    case BoardType::Pi500: w = 0; h = 1200; strict = true; break;
    case BoardType::RG351V:
    case BoardType::RG351P:
    case BoardType::OdroidAdvanceGo:
    case BoardType::OdroidAdvanceGoSuper:
...
bool ResolutionAdapter::GetCRTResolution(int& w, int& h)
{
  ...
  if(crt.HasForcedResolution(w,h))
  {
    { LOG(LogDebug) << "[ResolutionAdapter] forced resolution: "<< w << "x" << h ; }
    return true;
  }
  ...
  {
    if (CrtConf::Instance().GetSystemCRTAvoidFrontendSuperRez())
    {
      w = 320; h = 240;
    }
    else
