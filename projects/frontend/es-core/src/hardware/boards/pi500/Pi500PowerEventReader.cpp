...
Pi500PowerEventReader::Pi500PowerEventReader(HardwareMessageSender& messageSender)
...
Pi500PowerEventReader::~Pi500PowerEventReader()
{
  StopReader();
}

void Pi500PowerEventReader::StartReader()
{
  { LOG(LogDebug) << "[Pi500] Power button manager requested to start."; }
  Start("Pi500Power");
}

void Pi500PowerEventReader::StopReader()
{
  { LOG(LogDebug) << "[Pi500] Power button manager requested to stop."; }
  Stop();
}

void Pi500PowerEventReader::Break()
{
  if (mFileHandle >= 0)
  {
    { LOG(LogDebug) << "[Pi500] Breaking power button thread."; }
    mFileHandle = -1;
  }
}

void Pi500PowerEventReader::Run()
{
  { LOG(LogInfo) << "[Pi500] Running background power button manager."; }
  while(IsRunning())
  {
    mFileHandle = open(sInputEventPath, O_RDONLY);
    if (mFileHandle < 0)
    {
      { LOG(LogError) << "[Pi500] Error opening " << sInputEventPath << ". Retry in 5s..."; }
      sleep(5);
      continue;
    }
    ...
    while(IsRunning())
    {
      ...
      if (poll(&poller, 1, 100) != 1 || (poller.revents & POLLIN) == 0)
      {
        ...
        if (mWaitFor == WaitFor::Release)
        {
          ...
          if (elapsed >= 2000)
          {
            mSender.Send(BoardType::Pi500, MessageTypes::PowerButtonPressed, (int)elapsed);
      ...
      if (read(mFileHandle, &event, sizeof(event)) != sizeof(event))
      {
        ...
        if (mFileHandle >= 0)
        {
          { LOG(LogError) << "[Pi500] Error reading " << sInputEventPath << ". Retrying"; }
          continue;
        }
        // If file handle NOK, we're instructed to quit
        { LOG(LogInfo) << "[Pi500] Power event reader ordered to stop."; }
        break;
      }
