...
{
  if (AudioModeTools::CanPlayMusic() && (!PatronInfo::Instance().IsPatron() || ! IsPlaying()))
  {
...
{
  #ifdef OPTION_RECALBOX_NO_AUDIO
  return;
  #endif
  if (AudioModeTools::CanPlayMusic())
    PlayRandomMusic();
...
void AudioManager::PlayRandomMusic()
{
  ...
  // Remote music?
  RemotePlaylist::TrackInfo* remoteTrack = RemotePlaylist::Available() ? RemotePlaylist::Instance().GetNextTrack() : nullptr;
  ...
  { LOG(LogInfo) << "[AudioManager] Will play music from:" << log; }
  ...
  if (popupDuration != 0)
  {
    ...
    if (source == MusicSource::RemoteTrack)
    {
      ...
      text.Append(":\n").Append(remoteTrack->Name()).Append('\n')
          .Append('(').Append(remoteTrack->MixTape()).Append(_(" by ")).Append(remoteTrack->Artist()).Append(')');
...
{
  return mCurrentMusic != 0;
}

void AudioManager::ConfigurationAudioModeChanged(const AudioMode& audioMode)
{
  (void)audioMode;
  if (!IsPlaying() && AudioModeTools::CanPlayMusic())
    AudioManager::Instance().StartPlaying(ThemeManager::Instance().Main());
  else if (IsPlaying() && !AudioModeTools::CanPlayMusic())
