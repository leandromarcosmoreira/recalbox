             ...
             , public StringStatics
{
  public:
    ...
    /*!
     * @brief Delete the UTF8 chracter at the given position
     * @param position Char position
     * @return True if the char has been deleted, false otherwise (error, malformed string, ...)
     */
    bool DeleteUTF8(int position)
    {
      if (int nextPosition = position; NextUTF8(nextPosition)) { Delete(position, nextPosition); return true; }
      return false;
    }

    /*!
     * @brief Move the given cursor to the next UTF8 character
     * @param position In & out position
     * @return True of the next character has been found, false otherwise
     */
    bool NextUTF8(int& position) { return MoveForwardUTF8(position); }

    /*!
     * @brief Move the given cursor to the previous UTF8 character
     * @param position In & out position
     * @return True of the previous character has been found, false otherwise
     */
    bool PreviousUTF8(int& position) { return MoveBackwardUTF8(position); }

    /*!
     * @brief Move the cursor in the current UTF8 string
     * @param position Position to move
     * @param forward Tru to move forward, false to move backward
     * @return True of the move has been done, false on error
     */
    bool MoveUTF8(int& position, bool forward) { return forward  ? MoveForwardUTF8(position) : MoveBackwardUTF8(position); }
    ...
    /*!
     * @brief Uppercase a single unicode char
     * @param c Char to uppercase
     * @return Uppercase'd char
     */
    static Unicode UnicodeUppercase(Unicode c) { return sSmallToCapital[c & 0xFFFF]; }
    /*!
     * @brief Lowercase a single unicode char
     * @param c Char to lowercase
     * @return Lowercase'd char
     */
    static Unicode UnicodeLowercase(Unicode c) { return sCapitalToSmall[c & 0xFFFF]; }
  ...
  private:
    ...
    bool MoveForwardUTF8(int& from) const
    {
      const char* p = data() + from;
      char c = p[0];
      ...
      if ((c & 0x80) == 0) from += 1;
      // 110xxxxx, two bytes left in character
      else if ((c & 0xE0) == 0xC0) from += 2;
      // 1110xxxx, three bytes left in character
      else if ((c & 0xF0) == 0xE0) from += 3;
      // 11110xxx, four bytes left in character
      else if ((c & 0xF8) == 0xF0) from += 4;
      else return false;

      if (from >= Count()) { from = Count() - 1; return false; }
      return true;
    }

    bool MoveBackwardUTF8(int& from) const
    {
      for(;;)
      {
        if (--from < 0) { from = 0; return false; }
        if ((data()[from] & 0xC0) != 0x80) break;
      }
      return true;
    }
