...
*/
template</*IsPod*/typename T> class Array : private Allocator
{
  ...
  public:
    ...
    Array& operator =(const std::vector<T>& source) { Clear(); CopyFrom(source); return *this; }
#ifdef __cplusplus11
     ...
     */
    Array& Clear() { Resize(0); fCount = 0; return *this; }

    /*!
     * Delete item at the given index
     */
    Array& Delete(int index) { Contract(index, 1); return *this; }
    /*!
     * @brief Delete items starting at the given index
     * @param index Startingindex
     * @param count Numbner of item to remove
     */
    Array& Delete(int index, int count) { Contract(index, count); return *this; }
    /*!
     * @brief Delete last item
     */
    Array& DeleteLast() { if (--fCount < 0) fCount = 0; return *this; }

    /*!
     * @brief Reserve memory if the current count if less than the requested size
     * @param size Preallocated amount of item
     */
    Array& Reserve(int size) { if (size < fCount) this->Resize(size); return *this; }

    /*!
     * If the array size exceed the given size, it is truncated to the given size.
     * @param size Max size
     */
    Array& TruncateTo(int size) { if (fCount > size) { Resize(size); fCount = size; } return *this; }

    /*!
     * If the array size is less than the given size, it is expanded to the given size.
     * @param size Max size
     */
    Array& ExpandTo(int size) { if (fCount < size) { Resize(size); fCount = size; } return *this; }

    Array& Set(const T& item, int index)
    {
    ...
    Array& AddItems(const T* item, int count) { T* insertAt = Expand(fCount, count); memcpy(insertAt, item, sizeof(T) * count); return *this; }
    Array& Insert(const T& item, int index) { *Expand(index, 1) = item; return *this; }
    Array& Insert(const T& item, int index, int count) { T* insertAt = Expand(index, count); for(; --count >= 0;) insertAt[count] = item; return *this; }
    Array& InsertItems(const T* item, int index, int count) { T* insertAt = Expand(index, count); memcpy(insertAt, item, sizeof(T) * count); return *this; }
    ...
    void CopyFrom(const std::vector<T> source)                       { CopyFrom(source, fCount, 0, source.size()); }
    void CopyFrom(const std::vector<T> source, int index)            { CopyFrom(source, index, 0, source.size()); }
    void CopyFrom(const std::vector<T> source, int index, int count) { CopyFrom(source, index, 0, count); }
    void CopyFrom(const std::vector<T>& source, int index, int sourceindex, int count)
    {
      if ((unsigned int)index > (unsigned int)sourceindex) return;
      if (sourceindex + count > (int)source.size()) count = (int)source.size() - sourceindex;
      memcpy(Check(index, count), &source.data()[sourceindex], count * __OBJSZ);
    }
