...
template<typename T> class Queue : private Allocator
{
  ...
  public:
    ...
    T PopAt(int index)
    {
      ...
      if (In > mOut)
      {
        memmove(&__GET(index), &__GET(index + 1), __OBJSZ * ((In - index) - 1));
        mIn = In - 1;
      }
      else
      {
    ...
    bool Pop(T& thisone)
    {
      if (mIn > mOut)
      {
        for (int Out = mOut; Out < mIn; Out++)
          if (__GET(Out) == thisone) { (void)PopAt(Out - mOut); return true; }
      }
      for(int Out = Capacity(); --Out >= mOut; )
        if (__GET(Out) == thisone) { (void)PopAt(Out - mOut); return true; }
      for(int Out = mIn; --Out >= 0; )
        if (__GET(Out) == thisone) { (void)PopAt(Capacity() - mOut + Out); return true; }
      return false;
    }
