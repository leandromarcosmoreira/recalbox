...
template<class T> class StaticLifeCycleControler : public INoCopy
{
  ...
  public:
    ...
    /*!
     * @brief Check if instance available
...
StaticLifeCycleControler<T>::StaticLifeCycleControler(const char* name)
{
  if (sInstance != nullptr)
    { LOG(LogFatal) << "[StaticLifeCycleControler] Instance of " << name << " already exists!"; }
...
{
  if (sInstance != this)
    { LOG(LogFatal) << "[StaticLifeCycleControler] Abnormal destruction of " << sNameHolder.mClassName; }
...
T& StaticLifeCycleControler<T>::Instance()
{
  ...
  { LOG(LogFatal) << "[StaticLifeCycleControler] " << sNameHolder.mClassName << " not yet available!"; }
  __builtin_unreachable();
}

template<class T>
bool StaticLifeCycleControler<T>::Available()
{
