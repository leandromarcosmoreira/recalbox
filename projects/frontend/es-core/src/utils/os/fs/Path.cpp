...
void Path::Normalize()
{
  ...
  if (!(mPath.StartsWith(LEGACY_STRING("http://")) || mPath.StartsWith(LEGACY_STRING("https://"))))
...
String Path::FilenameWithoutDecorations() const
{
  String result(FilenameWithoutExtension());

  // Remove () starting from the end to minimize char moves
  for(int end = result.FindLast(')'); end >= 0; )
  {
    int start = result.FindLast('(', end);
    if (start < 0) break;
    while(result[++end] == ' '); // Trim right
    while(start != 0 && result[--start] == ' '); // Trim left
    ++start;
    result.erase(start, end - start);
    end = result.FindLast(')', start - 1);
  }
...
bool Path::RecurseDelete() const
{
  ...
  if(dir != nullptr)
  {
    ...
    while((entry = readdir(dir)) != nullptr)
    {
      ...
      if (entry->d_type == DT_DIR)
      {
        if (!item.RecurseDelete()) result = false;
        if (!item.Delete()) result = false;
      }
      else
        if (!item.Delete()) result = false;
    }
