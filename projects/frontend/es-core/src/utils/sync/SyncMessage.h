...
//! Simple concept of a max-sized POD class
template<class T>
concept IsPodMax = ((std::is_standard_layout_v<T> && std::is_trivial_v<T> && IsSizeCompatible<T>) || std::is_void_v<T>);

template<IsPodMax T> union SyncMessageBody
{
 ...
 */
template<IsPodMax T> class SyncMessage : StaticSyncMessageHolder
{
  public:
    ...
    //! Delayed frames. If not null, the message is not dispatched until this counter reaches 0
    int mFrameDelay;
