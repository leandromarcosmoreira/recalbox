...
class Files
{
  ...
  public:
    ...
    /*!
     * @brief Write the given string at the given offset of the given file or create it if it does not exist
     * @param path File path
     * @param data Data buffer to append
     * @param size Data size to append
     * @return True if the content has been saved
     */
    static bool WriteToFile(const Path& path, const void* data, int size, int offset);
     ...
     * @return True if the content has been saved
     */
    static bool WriteToFile(const Path& path, const String& content, int offset)
    {
      return WriteToFile(path, content.c_str(), content.Count(), offset);
    }
