...
class VideoEngine : public StaticLifeCycleControler<VideoEngine>, private Thread
{
  ...
  private:
    ...
    class PlayerContext
    {
      public:
        ...
        [[nodiscard]] bool IsInitialied() const { return AudioVideoContext != nullptr; }
    };
  ...
  public:
     ...
     */
    GLuint GetDisplayableFrame();

    /*!
     * @brief Get current image size
     * @return Image size
     */
    Vector2f GetDisplayableFrameSize() { return Vector2f((float)mTextureWidth, (float)mTextureHeight); }
