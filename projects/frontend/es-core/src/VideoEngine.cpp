...
void VideoEngine::StopVideo(bool waitforstop)
{
  ...
  {
    AquireTexture();
    Renderer::DestroyGLTexture(mTexture);
    mTexture = 0;
...
GLuint VideoEngine::GetDisplayableFrame()
{
  ...
  if (mContext.FrameRGB[frame] != nullptr)
  {
    ...
    if (rgba != nullptr)
    {
      if (mTexture == 0) mTexture = Renderer::CreateGLTexture(false);
      Renderer::UploadRGBA(mTexture, mContext.Width, mContext.Height, rgba);
      mTextureWidth = mContext.Width;
      mTextureHeight = mContext.Height;
    }
    ReleaseTexture();
  }
