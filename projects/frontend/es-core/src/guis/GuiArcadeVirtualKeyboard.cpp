...
GuiArcadeVirtualKeyboard::GuiArcadeVirtualKeyboard(WindowManager& window, const String& title, const String& initValue,
                                                   IVirtualKeyboardInterface* okCallback)
  : Gui(window)
  , mSavedAngles()
  , mTitle(title)
  ...
  , mWheelFont(&FontManager::Instance().FromFile(Path(":/vk.ttf"), GetFontSize(sWheelFontRatio), true))
  , mTitleFont(&FontManager::Instance().FromFile(Path(":/ubuntu_condensed.ttf"), GetFontSize(sTitleFontRatio), true))
  , mTextFont(&FontManager::Instance().FromFile(Path(":/ubuntu_condensed.ttf"), GetFontSize(sTextFontRatio), true))
  , mSelectedChar(TextureManager::Instance().Create(Path(":/plaincircle30.png"), TextureHolder::Properties::ImmediateLoad))
{
  ChangeWheel(0);

  // Compute editor bounds
  double ray = Renderer::Instance().DisplayWidthAsFloat();
  ray = ((ray / 2.0) - GetFontSize(sWheelFontRatio)) - (ray / 20.0f);
  float centerX = Renderer::Instance().DisplayWidthAsFloat() / 2.0f;
  float centerY = Renderer::Instance().DisplayHeightAsFloat() / 2.0f;

  float fontHeight  = (float)mTextFont->Height();
...
bool GuiArcadeVirtualKeyboard::ProcessInput(const InputCompactEvent& event)
{
  ...
  else
  {
    ...
    else if (event.CancelReleased() || event.StartReleased()) ValidateEditor();
...
void GuiArcadeVirtualKeyboard::AddCharacter(unsigned int unicode)
{
  ...
  if (mValidateCallback != nullptr)
    mValidateCallback->VirtualKeyboardTextChange(*this, String::JoinUnicodes(mText));
}

void GuiArcadeVirtualKeyboard::ValidateEditor()
{
  if (mValidateCallback != nullptr)
    mValidateCallback->VirtualKeyboardValidated(*this, String::JoinUnicodes(mText));

  Close();

  if (mValidateCallback != nullptr)
    mValidateCallback->VirtualKeyboardHide(*this);
}
...
{
  float baseline = (float)mTextFont->TopToBaseline();

  float offsetX = mInnerEditor.x - mOffsetInPixel;
  float offsetY = mInnerEditor.y + baseline;
  ...
  for(int i = 0; i < (int)mText.size(); i++)
  {
    ...
    float width = (float)mTextFont->CharWidth(unicode);
    ...
    mTextFont->DrawCharBaseline(unicode, (int)offsetX, (int)offsetY, sEditableTextColor);
    offsetX += width;
  }
  if (mCursor >= (int)mText.size())
  {
    Renderer::DrawRectangle(offsetX, mInnerEditor.y, (float)mTextFont->CharWidth(32), mInnerEditor.h, cursorColor);
  }
}
...
void GuiArcadeVirtualKeyboard::AdjustCursor(int cursorextend)
{
  ...
  float cursorWidth = (float)mTextFont->CharWidth(unicode);

  // Get current cursor position
  float cursorLeft = 0;
  for(int i = 0; i < mCursor; i++)
    cursorLeft += (float)mTextFont->CharWidth(mText[i]);
...
{
  float yray = Renderer::Instance().DisplayHeightAsFloat();
  yray = ((yray - GetFontSize(sWheelFontRatio)) / 2.0f) - (yray / 60.0f);
  yray = ((Renderer::Instance().DisplayHeightAsFloat() / 2.0f) - yray);

  int x1 = Math::roundi((Renderer::Instance().DisplayWidthAsFloat() / 2.0f) - (yray * 1.5f));
  int y1 = Math::roundi(-yray * 0.5f);
  int wh = Math::roundi((yray * 3.0f));

  mSelectedChar.Render(x1, y1, wh, wh, true, false);
}
...
void GuiArcadeVirtualKeyboard::RenderWheelChar(const Wheel& wheel, double centerx, double centery, int charindex, double xray, double yray, double sizeratio, double morphangle)
{
  ...
  int x = (int)((cos(angle - (Pi / 2.0)) * xray) + centerx);
  int y = (int)((sin(angle - (Pi / 2.0)) * yray) + centery);
  ...
  FontEffect effect;
  effect.SetScaling(wr, hr);

  mWheelFont->DrawChar(unicode, x - 2, y - 2, alpha, ::Alignment::Center, effect);
  mWheelFont->DrawChar(unicode, x + 0, y - 2, alpha, ::Alignment::Center, effect);
  mWheelFont->DrawChar(unicode, x + 2, y - 2, alpha, ::Alignment::Center, effect);
  mWheelFont->DrawChar(unicode, x + 2, y + 0, alpha, ::Alignment::Center, effect);
  mWheelFont->DrawChar(unicode, x + 2, y + 2, alpha, ::Alignment::Center, effect);
  mWheelFont->DrawChar(unicode, x + 0, y + 2, alpha, ::Alignment::Center, effect);
  mWheelFont->DrawChar(unicode, x - 2, y + 2, alpha, ::Alignment::Center, effect);
  mWheelFont->DrawChar(unicode, x - 2, y + 0, alpha, ::Alignment::Center, effect);

  mWheelFont->DrawChar(unicode, x, y, color, ::Alignment::Center, effect);
}
...
void GuiArcadeVirtualKeyboard::RenderWheel(const Wheel& wheel, double raymultiplier)
{
  ...
  for(int i=wheel.mWheelCharCount; --i >= 0;)
    if (i != selectedChar)
    {
      ...
      xray = (((xray - GetFontSize(sWheelFontRatio)) / 2.0f) - (xray / 60.0)) * raymc;
      double yray = Renderer::Instance().DisplayHeightAsFloat();
      yray = (((yray - GetFontSize(sWheelFontRatio)) / 2.0f) - (yray / 60.0)) * raymc;
...
void GuiArcadeVirtualKeyboard::RenderTextBox()
{
  ...
  // Render title
  mTitleFont->DrawText(mTitle, (int)mInnerEditor.x, (int)mOuterEditor.y - (int)(mTitleFont->Height() * 1.2f), sTitleTextColor, ::Alignment::TopLeft);

  // Clip text display
  Renderer::Instance().Clip(::Rectangle(mInnerEditor.x, mInnerEditor.y, mInnerEditor.w, mInnerEditor.h));
  // Render text
  RenderEditedString();
  // Pop clip
...
void GuiArcadeVirtualKeyboard::Render(const Transform4x4f& parentTrans)
{
  ...
  {
    if (mValidateCallback != nullptr)
      mValidateCallback->VirtualKeyboardShow(*this);
    mShown = true;
  }
}
...
{
  help.Clear()
      .Set(Help::Valid(), _("ADD CHARACTER"))
