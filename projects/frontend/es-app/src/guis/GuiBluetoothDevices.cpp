...
{
  // Title
  mTitle = std::make_shared<TextComponent>(mWindow, _("PAIR BLUETOOTH CONTROLLERS"), mMenuTheme.Title().font, mMenuTheme.Title().color, ::Alignment::Center);
  ...
  // Details
  mDetails = std::make_shared<MarkdownText>(mWindow, "", *mMenuTheme.Text().font, mMenuTheme.Text().color, ::Alignment::TopLeft);
...
void GuiBluetoothDevices::onSizeChanged()
{
  ...
  mGrid.setRowHeightPerc(0, (mTitle->getFont()->Height() + titleVerticalPadding) / mSize.y());
...
void GuiBluetoothDevices::FillDetails()
{
  ...
  else
  {
    ...
    String text;
    text.Append(_("Alias: **")).Append(device.Name).Append("**")
...
void GuiBluetoothDevices::FillDeviceList()
{
  ...
  if (!mDevices.empty())
  {
    ...
    {
      row.Clear();
      std::shared_ptr<Component> ed = std::make_shared<TextComponent>(mWindow, ComposeName(device), mMenuTheme.Text().font, mMenuTheme.Text().color, ::Alignment::CenterLeft);
  ...
  {
    std::shared_ptr<Component> ed = std::make_shared<TextComponent>(mWindow, _("NO DEVICE"), mMenuTheme.Text().font, mMenuTheme.Text().color, ::Alignment::CenterLeft);
    row.addElement(ed, true);
    mList->addRow(row, false, true);
  }
