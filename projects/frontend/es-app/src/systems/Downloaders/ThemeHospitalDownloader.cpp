...
#include "ThemeHospitalDownloader.h"
...
ThemeHospitalDownloader::ThemeHospitalDownloader(SystemData& themehospital, IGuiDownloaderUpdater& updater)
  : BaseSystemDownloader(themehospital, updater)
  , mSender(*this)
  , mSystem(themehospital)
  , mTotalSize(0)
  , mCurrentSize(0)
  , mGames(0)
{
...
void ThemeHospitalDownloader::DownloadAndInstall()
{
  mSender.Send(ThemeHospitalDownloadingGameState::Start);
  usleep(20000); // Let display refreshing
  { LOG(LogDebug) << "[ThemeHospitalDownloader] Download files for " << mSystem.FullName(); }
  ...
  if (output.IsEmpty()) { mSender.Send(ThemeHospitalDownloadingGameState::WriteOnlyShare); return; }

  // Get destination filename
  Path destination("/recalbox/share/system/download.tmp");
  { LOG(LogDebug) << "[ThemeHospitalDownloader] Target path " << destination.ToString(); }
  ...
  if (!mRequest.Execute(source, destination, this)) { mSender.Send(ThemeHospitalDownloadingGameState::DownloadError); return; }

  // Extract
  { LOG(LogDebug) << "[ThemeHospitalDownloader] Extracting game"; }
  Zip zip(destination);
  // Fill in storage
  ...
  for(int i = (int)mTotalSize; --i >= 0;)
  {
    ...
    { LOG(LogDebug) << "[ThemeHospitalDownloader] File " << destinationPath.ToString(); }
    ...
    else
    {
      ...
      if (!destinationPath.Exists())
      {
        ...
        { LOG(LogDebug) << "[ThemeHospitalDownloader] CRC Match"; }
      }
    }
    mCurrentSize++;
    mSender.Send(ThemeHospitalDownloadingGameState::Extracting);
    usleep(20000); // Let display refreshing
  }

  // Populate
  { LOG(LogDebug) << "[ThemeHospitalDownloader] Populate gamelist"; }
  ...
  if (result)
  {
    ...
    mCurrentSize = 0;
    mSender.Send(ThemeHospitalDownloadingGameState::UpdatingMetadata);
    ...
    for (const XmlNode fileNode : games.children())
    {
      ...
      if (String(fileNode.name()) == "game")
      {
        ...
        if (realGame == nullptr)
        { LOG(LogError) << "[ThemeHospitalDownloader] Cannot lookup real game!"; }
        else
        {
          // Merge data from
  ...
  else { LOG(LogError) << "[ThemeHospitalDownloader] Cannot load remote gamelist!"; }

  // Delete temp file
  (void)destination.Delete();
}

void ThemeHospitalDownloader::DownloadProgress(const HttpClient& http, long long currentSize, long long expectedSize)
{
  (void)http;
  // Store data and synchronize
  mTotalSize = expectedSize;
  mCurrentSize = currentSize;
  mSender.Send(ThemeHospitalDownloadingGameState::Downloading);
}

void ThemeHospitalDownloader::ReceiveSyncMessage(const ThemeHospitalDownloadingGameState& code)
{
  switch(code)
  {
    case ThemeHospitalDownloadingGameState::Start:
    {
      String title(_("DOWNLOADING GAME FOR %s"));
      mUpdater.UpdateTitleText(title.Replace("%s", mSystem.FullName()));
      mUpdater.UpdateMainText(_("Downloading Theme Hospital demo game from the official site. Please wait..."));
      break;
    }
    case ThemeHospitalDownloadingGameState::Downloading:
    {
    ...
    case ThemeHospitalDownloadingGameState::Extracting:
    {
      // Load size into progress bar component
      mUpdater.UpdateProgressbar(mCurrentSize, mTotalSize);
    ...
    case ThemeHospitalDownloadingGameState::UpdatingMetadata:
    {
      // Load size into progress bar component
      mUpdater.UpdateProgressbar(mCurrentSize, mTotalSize);
    ...
    case ThemeHospitalDownloadingGameState::WriteOnlyShare:
    {
      mUpdater.UpdateETAText(_("Can't write game to share!"));
      break;
    }
    case ThemeHospitalDownloadingGameState::DownloadError:
    {
...
void ThemeHospitalDownloader::Completed(bool stopped)
{
