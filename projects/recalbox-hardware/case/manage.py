...
def DetectRaspberryPiTouchDisplay():
    logger.hardlog("Trying rpi 7\" display")
    try:
        with open("/sys/firmware/devicetree/base/panel_disp@1/compatible", "r") as f:
            if f.readline().find("7inch-dsi") >= 0:
                return cases.RPITOUCHDISPLAY
    except:
        pass
    return cases.NONE

def DetectSteamDeck():
    logger.hardlog("Trying Steam Deck")
    with open("/sys/devices/virtual/dmi/id/board_name", "r") as f:
        ...
        if board_name == "Jupiter":
            return cases.STEAMDECKLCD
        if board_name == "Galileo":
            return cases.STEAMDECKOLED
    return cases.NONE
# --------- Main

manualCases = (cases.SUPERPI4CASE, cases.NESPI4MANUAL, cases.PISTATION, cases.ARGONONE,
               cases.ARGONONEV3, cases.NESPICASEPLUS, cases.SUPERPICASE, cases.MEGAPICASE,
               cases.RPITOUCHDISPLAY)
...
def Identify(previousCase):
    ...
    if board in ("rpi3", "rpi4", "rpi4_64", "rpi5_64") and case in (cases.NONE, cases.RPITOUCHDISPLAY):
        case = DetectRaspberryPiTouchDisplay()

    if board in ("x86_64") and case in (cases.NONE, cases.STEAMDECKLCD, cases.STEAMDECKOLED):
        case = DetectSteamDeck()
