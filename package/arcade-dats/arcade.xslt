...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
   ...
   <xsl:template match="datafile|mame">
      ...
      <xsl:value-of select="'|'"/>
      <!--manufacturer du jeu-->
      <xsl:value-of select="'manufacter'" /> 
      <xsl:value-of select="'&#10;'"/>

	   <xsl:apply-templates select="game">
         <xsl:with-param name="isfba" select="substring(header/name,1,8)='FB Alpha'" />
         <xsl:with-param name="isfbn" select="substring(header/name,1,13)='FinalBurn Neo'" />
      </xsl:apply-templates>
   ...
   <xsl:template match="game|machine">
      ...
      <!--width du jeu ref lastmame-->
      <xsl:choose>
         <xsl:when test="video/@width">
            <xsl:value-of select="video/@width" />
         </xsl:when>
         <xsl:otherwise>
            <xsl:if test="not ($isfba) and not ($isfbn)">
      ...
      <!--refresh du jeu ref lastmame-->
      <xsl:choose>
         <xsl:when test="video/@refresh">
            <xsl:value-of select="video/@refresh" />
         </xsl:when>
         <xsl:otherwise>
            <xsl:if test="not ($isfba) and not ($isfbn)">
